...
| Evaluation + content checks                   | 🟢 `FULLY_FUNCTIONAL` | 16 eval/content checks per system (Linux: 17, assertion checks are Linux-only; x86_64-linux adds the VM test) + formatters; all Nix attribute-equality (`assertEq`), each family kill-switch tested |
