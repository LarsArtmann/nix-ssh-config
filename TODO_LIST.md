...
| Restore content assertions cut in the flake-parts migration (custom port, authorizedKeys, banner, crypto lists, `extraSettings` merge, HM host-block content) | 🔴 `TODO` | High   | 2h     | `flake.nix:109-131` keeps only 2 content checks (password auth, root login); 12 were dropped at `e910e78`    |
