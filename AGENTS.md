...
| `checks.<system>.*`                   | 21 eval/content checks (22 on Linux: assertion checks are Linux-only) + one `format` (treefmt) gate; on x86_64-linux additionally `nixos-vm-sshd` (QEMU integration test — its client node runs the HM client module itself via HM-in-NixOS, so client config gets runtime proof). `docs-option-inventory`/`docs-check-count` red-flag README/FEATURES drift when these change |
...
`programs.ssh.settings.<block>` is `{ before, after, data, header }`; the actual directives live under `.data` (same as the old `matchBlocks`). The flake's `hmBlock` helper unwraps it. If HM changes representation, that helper is what needs updating. Additionally, `.data` itself carries a `header` key (`"Host <alias>"`) next to the directives — the table-driven `hostFixtures` rows in `tests/checks.nix` assert full-block equality INCLUDING that header; comparing directives alone will not match (found 2026-09-18 by the first full-block run).
...
`PerSourcePenalties = true` (a module default) makes sshd record a **deferred 5-second penalty** per client IP after an auth failure — visible in the VM transcript as `srclimit_penalise: ... deferred penalty of 5.000 seconds`. Observed behavior (2026-08-29, prompt-path work): the subtest immediately after the wrong-key rejection still connected and succeeded — a _deferred_ penalty does not drop the next connection within the window, so the wrong-key test cannot flake against its neighbors. The kbd-server subtests are immune anyway: it is a second sshd with its own penalty state. When ADDING subtests that expect repeated auth failures against the same node, put them before network-success probes or space them past 5s; if a mysterious `Connection refused`/drop shows up after a failure-heavy subtest, suspect the penalty window first, not the config. Related flake source, seen once on 2026-08-29: `pam_unix`'s fail-delay (~2-4s per failed prompt auth) can stretch a keyboard-interactive refusal until sshd closes the connection before the client prints `Permission denied` — refusal assertions therefore accept `Permission denied` **or** `Connection closed` (the property is _not accepted_, not the exact string). Also note the nixos test driver reserves the name `log` (an AbstractLogger) — a testScript variable named `log` fails the driver's Python type-check with a confusing error; use `jlog` or similar. Second flake class, seen 2026-09-18 after the HM-in-NixOS client made the script faster: `start_all()` does NOT guarantee every node finished booting — a node first contacted late in the script (kbd-server) can still be mid-boot when its subtest runs, surfacing as `Connection refused`. Every node needs its own `wait_for_unit`/`wait_for_open_port` before first contact; the kbd subtest now waits explicitly.

### BuildFlow `nix-checker` port-collision findings are false positives (skip is deliberate)

`buildflow`'s in-process nix-checker emits 24 error-severity `port-collision` findings against `examples/client.nix`, `modules/home-manager/ssh.nix`, and `tests/checks.nix` ("Port 2222 is assigned in 4 places..."). All are false positives: client configs and HM host blocks never bind a port, and repeated ports inside `tests/checks.nix` belong to independent `nixosTest` VMs with isolated network namespaces — no two of the flagged assignments can ever collide on one host. Do NOT renumber test ports or example ports to please the analyzer. `.buildflow.yml` exists solely to `skip_steps: [nix-checker]` (whole-tool skip is BuildFlow's only suppression mechanism; there is no per-rule ignore key). Real nix quality stays enforced by the nix-build/statix/treefmt steps and CI. Before un-skipping, check BuildFlow upstream for per-rule suppression or a scope-aware rule (surfaced 2026-09-18, first run where nix-checker executed — earlier runs died at nix-build before this step).
