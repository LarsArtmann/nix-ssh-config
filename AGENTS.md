...
| `checks.<system>.*`                   | 20 eval/content checks (21 on Linux: assertion checks are Linux-only) + one `format` (treefmt) gate; on x86_64-linux additionally `nixos-vm-sshd` (QEMU integration test). `docs-option-inventory`/`docs-check-count` red-flag README/FEATURES drift when these change |
